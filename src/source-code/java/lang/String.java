...
public final class String
    ...
    public String(char value[]) {
        // 调用 Arrays.copyOf() 方法，将传入的字符数组赋值给
        // 推测 这样做的目的是为了实现深拷贝
    ...
    public String(char value[], int offset, int count) {
        ...
        // 经过一系列的参数合法性判断，最后调用 Arrays.copyOfRange() 方法
    ...
    public String(StringBuffer buffer) {
        // 为什么这里要加锁？其他地方不加
        // 是因为 StringBuffer 本身就是保证线程安全的，所以出现的地方也要线程安全？
        synchronized (buffer) {
    ...
    public boolean equals(Object anObject) {
        // 首先直接用 == 比较
        // 如果相等，就说明是同一个对象（因为调用的是 String，所以相等的时候参数肯定也是 String），直接返回 true
        if (this == anObject) {
            return true;
        }
        // 判断参数的类型是否是 String
        if (anObject instanceof String) {
            ...
            int n = value.length;
            // 判断两个字符串长度是否相等
            // 这里为什么用 anotherString.value.length，而不是 anotherString.length()？
        ...
        // 如果参数不是 String，不可能相等，直接返回 false
        return false;
    }
    ...
    public int compareTo(String anotherString) {
        ...
        int lim = Math.min(len1, len2);// 两个字符串长度的较小值
        char v1[] = value;
        char v2[] = anotherString.value;
    ...
    public boolean regionMatches(boolean ignoreCase, int toffset,
        ...
        char ta[] = value;// 当前字符数组
        int to = toffset;
        char pa[] = other.value;// 要比较的字符数组
        int po = ooffset;
        // Note: toffset, ooffset, or len might be near -1>>>1.
        // 校验几个参数的合法性，不合法直接返回 false
        if ((ooffset < 0) || (toffset < 0)
                || (toffset > (long) value.length - len)
                || (ooffset > (long) other.value.length - len)) {
            return false;
        ...
        while (len-- > 0) {
            ...
            if (ignoreCase) {
                ...
                // continue.
                // 如果忽略大小写比较，先将字符转为大写再比较
                // 直接用 == 是因为跟 Integer 一样有缓存吗？
